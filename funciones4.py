...
def leer_labels_kitti(ruta, estructura=1, idImagen=0, Tr_velo_to_cam=None, R0_rect=None):
    etiquetas = []
    with open(ruta, 'r') as file:
        for linea in file:
            datos = linea.split()
            if datos[0] != 'DontCare':
                if estructura == 1:
                    etiqueta = {
                        ...
                        'ty': float(datos[12]),
                        'tz': float(datos[13]),
                        'height': float(datos[8]),
                        'width': float(datos[9]),
                        'length': float(datos[10]),
                        'rot_y': float(datos[14])
                    }
                    # --- CONVERTIR A LIDAR ---
                    if Tr_velo_to_cam is not None and R0_rect is not None:
                        label_xyz_kitti = [etiqueta['tx'], etiqueta['ty'], etiqueta['tz']]
                        label_xyz_lidar = label_kitti_a_lidar(label_xyz_kitti, Tr_velo_to_cam, R0_rect)
                        etiqueta['tx'] = label_xyz_lidar[0]
                        etiqueta['ty'] = label_xyz_lidar[1]
                        etiqueta['tz'] = label_xyz_lidar[2]
                    etiquetas.append(etiqueta)
                elif estructura == 2 and int(datos[0]) == idImagen:
                    etiqueta = {
                        'id': datos[1].lower(),
...
def asociar_estimaciones_con_labels2(
    centro_lidar, etiquetas, contador, diccionario, idImagen, puntos_recortados, Tr_velo_to_cam, R0_rect
):
    """
    Asocia una estimación (centro_lidar) con la etiqueta KITTI más cercana (convertida a sistema LiDAR) y guarda en diccionario.
    """
    usadas = set() 
    mejor_match = None
    menor_distancia = float('inf')
    """for i, etiqueta in enumerate(etiquetas):
        if i in usadas:
            continue
        label_lidar = label_kitti_a_lidar([etiqueta['tx'], etiqueta['ty'], etiqueta['tz']], Tr_velo_to_cam, R0_rect)
    ...
    for i, etiqueta in enumerate(etiquetas):
        if i in usadas:
            continue
        label_lidar = np.array([etiqueta['tx'], etiqueta['ty'], etiqueta['tz']])
        distancia = np.linalg.norm(centro_lidar[:3] - label_lidar)
        if distancia < menor_distancia:
            mejor_match = (i, etiqueta, label_lidar, distancia)
            menor_distancia = distancia

    if mejor_match and menor_distancia <= 6:  # threshold configurable
        usadas.add(mejor_match[0])
        ...
        diccionario[idImagen][contador]["points"] = puntos_recortados
        diccionario[idImagen][contador]["label"] = etiqueta_label
        print(f"Objeto asociado: {identificador} (distancia: {distancia:.2f}m)")
        return identificador
    else:
        print("No se ha encontrado asociación posible para este objeto")
        return None

def asociar_estimaciones_con_labels3(centro_3d, etiquetas, contador, diccionario, idImagen, puntos_recortados ):
    """
    Asocia las estimaciones (x, y, z) con las líneas del archivo KITTI comparando posiciones.
    Imprime las asociaciones id= id con línea correspondiente y guarda los resultados en un diccionario.
    ...
    for idx, (coord_x, coord_y, coord_z) in enumerate(zip(x, y, z)):
        ...
        for i, etiqueta in enumerate(etiquetas):
            ...
            """distancia = np.sqrt((coord_x - etiqueta['tz'])**2 +
                                ...
                                (coord_z - etiqueta['ty'])**2)  """
            label_lidar = label_kitti_a_lidar([etiqueta['tx'], etiqueta['ty'], etiqueta['tz']], Tr_velo_to_cam, R0_rect)
            distancia = np.linalg.norm(centro_3d[:3] - label_lidar)
            if distancia < menor_distancia:
                mejor_match = (i, etiqueta)
                menor_distancia = distancia
...
def inferencia2(imagen, idImagen, ruta_label, ruta_lidar, diccionario, ruta_calibracion, guardarImagenes = False, info=0, estrcuturaLabel =1):
    ...
    todos_los_puntos = []
    etiquetas = leer_labels_kitti(ruta_label, estrcuturaLabel, int(idImagen), Tr_velo_to_cam, R0_rect)
    #print(etiquetas)
    diccionario_centros= {}
    datos_imagen = {}
    # Verificar si la imagen se cargó correctamente
            ...
            for box in r.boxes:
                ...
                if centro_3d is not None and len(puntos) > 0:
                    ...
                    if len(puntos_cluster_cercano) > 0:
                        ...
                        identifiador = asociar_estimaciones_con_labels2(centro_3d, etiquetas, contador, diccionario, idImagen, puntos_recortados, Tr_velo_to_cam, R0_rect)
                        for i, etiqueta in enumerate(etiquetas):
