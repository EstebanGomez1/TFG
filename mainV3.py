...
idImagen = "000000"
idSeccion = "0018" # faltan la 19 y 20
ruta_kitti = 'datos'
...
end_id = cantidad  # Representa 000700
...
for i in tqdm(range(start_id, end_id + 1)):
    try:
        ...
        if os.path.exists(ruta_imagen) and os.path.exists(ruta_label) and os.path.exists(ruta_lidar) and os.path.exists(ruta_calibracion):
            ...
            diccionario = funciones4.inferencia2(imagen, idImagen, ruta_label, ruta_lidar, diccionario, ruta_calibracion, True, 0, 2) #Con 0 no info, 1 toda info, 2 info imagenes ; 1 estrucura normal, 2 estructura tracking
